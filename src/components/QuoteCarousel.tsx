...
const QuoteCarousel: React.FC<QuoteCarouselProps> = ({
  ...
  return <div className={cn("w-full py-16 bg-gradient-to-b from-background to-secondary/20", className)}>
      <div className="max-w-4xl mx-auto px-4">
        <h2 className="text-2xl font-semibold text-center mb-12 text-foreground/90"></h2>
        
        <Carousel setApi={setApi} className="w-full mx-auto" opts={{
        align: "center",
        loop: true
      }}>
          <CarouselContent>
            {quotes.map((quote, index) => <CarouselItem key={index}>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {quotes.map((_, index) => <button key={index} className={cn("w-3 h-3 rounded-full transition-all duration-500", currentIndex === index ? "bg-primary scale-125" : "bg-muted hover:bg-primary/50")} onClick={() => api?.scrollTo(index)} aria-label={`Go to quote ${index + 1}`} />)}
          </div>
        </Carousel>
      </div>
    </div>;
